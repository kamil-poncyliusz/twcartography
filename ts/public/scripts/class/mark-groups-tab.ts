...
const generateMarkGroupRowInnerHTML = function (group: MarkGroup, tribes: { [key: string]: Tribe }): string {
  ...
  for (let tribeId of group.tribes) {
    ...
    innerHTML += `<button class='mark block delete-button' title='${translation.delete.toLowerCase()} ${tribe.name}'>${tribe.tag}</button>`;
  }
  ...
  innerHTML += `<td class='group-name'><input type='text' value='${group.name}' placeholder='${translation.name}'></td>`;
  innerHTML += `<td><input type='color' title='${translation.markGroupColorInputTooltip}' value='${group.color}' style='background-color:${group.color};'></td>`;
  innerHTML += `<td>${group.tribes.length}</td><td>${players}</td><td>${villages}</td><td>${points}</td>`;
  innerHTML += `<td><button class='delete-group delete-button' title='${translation.delete.toLowerCase()}'>X</button></td>`;
  return innerHTML;
};
